...
import weatherService.weatherServiceTest;
...
//			System.out.println("用户消息:"+Content);
			if (Content.equals("音乐")) {
				...
				if (Content.contains(":")) {
					String proviceName = Content.substring(Content.lastIndexOf(":") + 1, Content.lastIndexOf(":") + 3);
					String cityName = Content.substring(Content.lastIndexOf(":") + 4, Content.lastIndexOf(":") + 6);
					String placeName = Content.substring(Content.lastIndexOf(":") + 7, Content.lastIndexOf(":") + 9);
					System.out.println(proviceName+cityName+placeName);
					
					JSONObject oneDayInfo = null;
					JSONObject twoDayInfo = null;
					JSONObject threeDayInfo = null;

					String info = weatherServiceTest.searchWeather(proviceName, cityName, placeName);
					...
					result = buildTextMessage(map, weatherInfo.toString());
//                    
//                    result = buildTextMessage(map,weatherInfo);
				} else {
					String notice = "查询天气的正确姿势: 天气:省份，城市名，地区名(例如:湖北省，襄阳市，襄城区)\n请客官输入正确的格式哟~";
